...
import { QuestionDataSchema } from './QuestionModel';
import { PaginationSchema } from '../../application/base/PaginationModel';

export class QuestionRestController extends ExpressRestControllerBase {
    constructor(public readonly repository: QuestionRepositoryPort) {
        super();
    }
    ...
    public override async Post(req: Request, res: Response): Promise<Response> {
        ...
        const data = QuestionDataSchema.validate(req.body);
        if (data.error) {
